...
class FrontendController extends Controller
{
    ...
    public function quests(Request $request)
    {
        ...
        if($challenge->coin_sku != 'BITP'){
            ...
            if((int)$qc < (int)$challenge->qc){
                ...
                $response = Http::post('http://wallet.bitpool.gg/api/credit-qc', [
                    'id' => $uid,
                    'amount' =>$challenge->qc
                ]);
    ...
    public function register(Request $req){
        ...
        $response = Http::withOptions([
            'debug' => false,
            'verify' => false,
        ])->post('https://app.bitsport.gg/api/bitpool/register', [
            'email' => $email,
            'password' => $password,
            'fname' => $fname,
            'lname' => $lname,
    ...
    public static function getUSDG(){
        ...
        $response = Http::withOptions([
            'debug' => false,
            'verify' => false,
        ])->post('https://wallet.bitpool.gg/api/get-usdg', [
            'id' => $uid
        ]);
        ...
        if($response == null){
            session()->put('isnew', true);
            $response = array('usdg' => 0, 'qc' => 0);
